...
public class PlayerNetworkActions : NetworkBehaviour
{
    ...
    public void Start()
    {
        ...
        if (!isLocalPlayer)
        {
            ...
            Debug.Log(gameObject.tag + particleEffects.Length.ToString());
            //turning off star particle
            particleEffects[0].Stop();
            particleEffects[0].Clear();
            DisableComponents();
            return;
        }
        else
    ...
    //Shooting mehtods
    public void ShootKurla() { CmdShoot(shotSpawn.position, shotSpawn.rotation, gameObject.tag + "_" + this.GetComponent<NetworkIdentity>().netId.ToString()); }
    [Command]
    ...
    public void UpdateHP(){
        Debug.Log("DUPAAAAAA HA PE");
        float maxHealth = shipStats._shipStats.maxHealth;
        Debug.Log("HP: " + curHealth.ToString() + "/" + maxHealth.ToString());
        float HPratio = curHealth/maxHealth;
        UI.transform.GetChild(0).GetChild(0).GetComponent<Text>().text = curHealth.ToString() + "/" + maxHealth.ToString();
        if(HPratio > (2f/3f)){
            UI.transform.GetChild(0).GetComponent<RawImage>().texture = spritesHP[0];
            UI.transform.GetChild(0).GetChild(0).GetComponent<Text>().color = Color.green;
        }
        else if(HPratio > (1f/3f)){
            UI.transform.GetChild(0).GetComponent<RawImage>().texture = spritesHP[1];
            UI.transform.GetChild(0).GetChild(0).GetComponent<Text>().color = new Color(1f, 0.6471f, 0f);
        }
        else{
            UI.transform.GetChild(0).GetComponent<RawImage>().texture = spritesHP[2];
            UI.transform.GetChild(0).GetChild(0).GetComponent<Text>().color = Color.red;
        }
    }
    Transform FindChild(Transform Parent, string key){
    ...
    public void TakeDamage(float amount, string _attacker)
    {
        ...
        string thisPlayerID = gameObject.tag + "_" + gameObject.GetComponent<NetworkIdentity>().netId.ToString();
        curHealth -= amount;
        if (curHealth <= 0f)
        {
            curHealth = 0f;
